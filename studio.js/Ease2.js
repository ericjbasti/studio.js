...
_s.DisplayObject.prototype.addTween=function(t,a,d,next,recycle){
...
//	we need to uniquely identify the tween so we can delete it once done.
	...
	this.tween[name]=function(){
		if(w.ctx.then>=start && w.ctx.then<=(start+d)){
			cur=w.time+=((w.ctx.timeDelta)*w._speed);
			if(d>cur){
				for (var i in a){
...
_s.DisplayObject.prototype.time=0;


// addTweenLine() adds a tween to the object but makes it so it is retained.
